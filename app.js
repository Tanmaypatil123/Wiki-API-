...
app
  ...
  .post(function (req, res) {
    ...
    newArticle.save(function (err) {
      if (!err) {
        res.send("Successfuly inserted the data");
      } else {
        res.send(err);
      }
    });
  ...
  .delete(function (req, res) {
    Article.deleteMany(function (err) {
      if (!err) {
        res.send("Successfully delted the data");
      } else {
        res.send(err);
      }
    });
...
.put(function(req,res){
  Article.update({title: req.params.articleTitle},{title:req.body,content},{overwrite:true},function(err){
    if(!err){
